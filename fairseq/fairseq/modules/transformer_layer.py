...
class TransformerGraphEncoderLayerBase(nn.Module):
    """Encoder layer block.
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        ...
        self.structure_w_v = nn.Linear(64, 1024)
        self.structure_w_k = nn.Linear(64, 1024)
    ...
    def forward(
        ...
        if not gamma == None:
            ...
            for i in range(x.size(0)):
                single_x, _ = self.self_attn(
                        query=x[i].unsqueeze(0),
                        key=x + self.structure_w_k(gamma[i]),
                        value=x + self.structure_w_v(gamma[i]),
                        key_padding_mask=encoder_padding_mask,
                        need_weights=False,
                        attn_mask=attn_mask,
                    )
                distributed_x.append(single_x[0])
            x = torch.stack(distributed_x)
        else:
            x, _ = self.self_attn(
                query=x,
                key=x,
